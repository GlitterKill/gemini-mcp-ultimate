...
export interface ToolArguments {
  ...
  // --> Ask Gemini tool
  session_id?: string | number; // For resuming sessions
  approval_mode?: 'default' | 'auto_edit' | 'yolo'; // Autonomy control
  include_directories?: string[]; // Context targeting
  output_format?: 'text' | 'json' | 'stream-json'; // Output format
  experimental_acp?: boolean; // ACP mode
  allowed_tools?: string[]; // Specific tools to allow

  // --> Manage Extensions tool
  action?: string;
  target?: string;
  all?: boolean;
  scope?: string;

  [key: string]: string | boolean | number | undefined | string[]; // Allow additional properties
}